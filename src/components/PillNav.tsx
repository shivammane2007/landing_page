...
const PillNav = ({
  ...
  return (
    <div className="pill-nav-container w-full flex justify-center">
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        {isRouterLink(items?.[0]?.href) ? (
          ...
          >
            {typeof logo === 'string' ? <img src={logo} alt={logoAlt} ref={logoImgRef} /> : <div ref={logoImgRef} className="w-full h-full">{logo}</div>}
          </Link>
        ) : (
          <a
            className="pill-logo"
