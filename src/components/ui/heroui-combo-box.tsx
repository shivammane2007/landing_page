...
.list-box-item[data-selected="true"] { background: #2563eb !important; color: #ffffff !important; }
.list-box-item[data-selected="true"] .list-box-item__indicator { color: #ffffff !important; }
.list-box-item[data-selected="true"]:hover, .list-box-item[data-selected="true"][data-highlighted="true"] { background: #1d4ed8 !important; }
...
function ComboBoxRoot({
  ...
  const shouldShowItem = React.useCallback(
    (textValue: string) => {
      ...
      if (selectedKey != null && items.get(selectedKey) === inputValue) return true;
      if (defaultFilter) return defaultFilter(textValue, inputValue);
      return textValue.toLowerCase().includes(inputValue.toLowerCase());
    },
    [defaultFilter, inputValue, selectedKey, items],
  );
